...
- Name: portfolio.spec.ts >> Chat Functionality (requires backend) >> chat should send message
- Location: tests/portfolio.spec.ts:186:3
...
Call log:
  - waiting for locator('button').filter({ has: locator('svg') }).last()
    - locator resolved to <button class="fixed bottom-6 right-6 w-16 h-16 rounded-full shadow-2xl transition-all duration-300 transform hover:scale-110 active:scale-95 z-40 flex items-center justify-center font-bold text-white bg-gradient-to-br from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 animate-bounce">…</button>
